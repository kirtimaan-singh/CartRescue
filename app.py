...
<style>
    ...
    .brand-name {{ font-weight: 700; font-size: 20px; color: {TEXT_PRIMARY}; }}
    ...
    .icon-btn {{
        ...
        font-size: 15px; color: {TEXT_PRIMARY};
    }}

    .subtitle {{ font-size: 14px; color: {TEXT_SECONDARY}; margin-bottom: 14px; }}
    .section-heading {{ font-weight: 700; font-size: 20px; color: {TEXT_PRIMARY}; margin: 6px 0 10px 0; }}
    .timestamp-row {{ font-size: 12px; color: {TEXT_SECONDARY}; margin-bottom: 14px; }}
    ...
    .metric-card h4 {{ color: {TEXT_SECONDARY} !important; }}
    .status-dot {{ display: inline-block; width: 10px; height: 10px; border-radius: 50%; margin-right: 8px; }}

    .panel {{ background-color: {SURFACE}; border: 1px solid {BORDER}; border-radius: 14px; padding: 16px 20px; margin-bottom: 16px; }}
    .alert-row {{ display: flex; align-items: center; gap: 10px; padding: 10px 0; border-bottom: 1px solid {BORDER}; font-size: 14px; color: {TEXT_PRIMARY}; }}
    ...
    .stButton > button {{ background-color: {ACCENT}; color: #06251C !important; border: none; font-weight: 700; border-radius: 8px; }}
    .stButton > button:hover {{ background-color: {ACCENT_DIM}; color: #ffffff !important; }}
    .stDownloadButton > button {{ background-color: {SURFACE_ALT}; color: {TEXT_PRIMARY} !important; border: 1px solid {BORDER}; font-weight: 700; border-radius: 8px; }}
    .stDownloadButton > button:hover {{ border-color: {ACCENT}; color: {ACCENT} !important; }}

    .stAlert p {{ color: {TEXT_PRIMARY} !important; }}
    ...
    .auth-wrapper {{
        ...
        max-width: 900px; margin: 30px auto; background-color: {SURFACE};
    }}
    .auth-brand-panel {{
        flex: 1; padding: 40px 34px; background: linear-gradient(160deg, #0B2A22 0%, #0E1013 75%);
        display: flex; flex-direction: column; justify-content: center; border-right: 1px solid {BORDER};
    }}
    .auth-form-panel {{ flex: 1.15; padding: 40px 38px; }}
    .auth-feature-row {{ display: flex; align-items: flex-start; gap: 10px; margin-bottom: 12px; font-size: 13.5px; color: {TEXT_SECONDARY}; }}
...
# ----------------- SESSION STATE -----------------
for key, default in [
    ("authenticated", False), ("seller_name", ""), ("marketplace_default", "Amazon"), ("last_refresh", datetime.now()),
]:
...
def render_auth_screen():
    ...
    with left:
        ...
        <div class="auth-brand-panel">
            <div class="logo-mark" style="margin-bottom:20px;">C</div>
            <div style="font-weight:700; font-size:22px; color:{TEXT_PRIMARY}; margin-bottom:10px;">
                Know when a sale is about to slip away.
            </div>
            <div class="auth-feature-row"><span class="tick">✓</span> Real-time cart risk scoring</div>
            <div class="auth-feature-row"><span class="tick">✓</span> Revenue-at-risk, net of commission</div>
            <div class="auth-feature-row"><span class="tick">✓</span> SKU-level breakdown</div>
    ...
    with right:
        ...
        st.markdown(f'<div style="font-weight:700; font-size:18px; color:{TEXT_PRIMARY}; margin-bottom:18px;">Welcome back</div>', unsafe_allow_html=True)
...
def generate_ecommerce_data(n_samples=1000):
    ...
    skus = np.random.choice(
        ["Classic Cotton Kurta", "Running Shoes Pro", "Wireless Earbuds X2", "Stainless Steel Bottle",
         ...
         "Yoga Mat Premium", "Bluetooth Speaker Mini"], size=n_samples
    )
...
st.sidebar.markdown(
    f"""<div class="seller-badge">
        ...
        <div style="font-size:12px; color:{TEXT_SECONDARY}; margin-top:10px;">{sessions_used:,} / {plan_limits[plan_tier]:,} sessions this month</div>
...
st.markdown(f'<div class="timestamp-row"><span class="dot">●</span> Last refreshed: {st.session_state.last_refresh.strftime("%d %b %Y, %I:%M %p")}</div>', unsafe_allow_html=True)
...
with tab1:
    ...
    with col1:
        ...
        st.markdown(f"""<div class="metric-card"><h4>Abandonment Risk</h4>
            <h1 style='color:{risk_color}; margin:10px 0 0 0; font-size:36px;'>{prob_abandon:.1%}</h1></div>""", unsafe_allow_html=True)
    with col2:
        st.markdown(f"""<div class="metric-card"><h4>Checkout Likelihood</h4>
            <h1 style='color:{ACCENT}; margin:10px 0 0 0; font-size:36px;'>{prob_checkout:.1%}</h1></div>""", unsafe_allow_html=True)
    with col3:
        if prob_abandon > 0.7:
            status_label, status_color = "High Risk", NEGATIVE
        elif prob_abandon > 0.4:
            status_label, status_color = "Moderate Risk", WARNING
        else:
            status_label, status_color = "Low Risk", ACCENT
        st.markdown(f"""<div class="metric-card"><h4>Session Status</h4>
            <div style="margin-top:18px;"><span class="status-dot" style="background-color:{status_color};"></span>
            <span style='color:{status_color}; font-size:16px; font-weight:700;'>{status_label}</span></div></div>""", unsafe_allow_html=True)

    net_margin_factor = 1 - (commission_rate / 100)
    revenue_at_risk = input_cart_val * prob_abandon * net_margin_factor
    st.write("")
    st.caption(f"Revenue at risk (net of {commission_rate}% commission): ₹{revenue_at_risk:,.0f}")

    st.write("---")
    st.markdown('<p class="section-heading">Recommended Action</p>', unsafe_allow_html=True)
    if prob_abandon >= 0.7:
        st.error(f"Offer a ₹{int(input_cart_val * 0.10)} coupon and surface support/chat before this cart is lost.")
    elif 0.4 <= prob_abandon < 0.7:
        st.warning("Highlight free shipping and trust badges near checkout.")
    else:
        st.success("No intervention needed — preserve margin, consider a light cross-sell.")
    ...
    for i, (_, r) in enumerate(recent_high_risk.iterrows()):
        alert_rows += (f'<div class="alert-row"><span class="status-dot" style="background-color:{NEGATIVE};"></span>'
                       f'{r["SKU"]} — ₹{r["Cart_Value_INR"]:,.0f}<span class="alert-time">{(i + 1) * 2}h ago</span></div>')
    st.markdown(f'<div class="panel">{alert_rows}</div>', unsafe_allow_html=True)

# ----------------- TAB 2: STORE PERFORMANCE -----------------
with tab2:
    st.markdown(f'<p class="section-heading">Store performance — {seller_name}</p>', unsafe_allow_html=True)
    ...
    st.write("---")
    category_benchmarks = {"Fashion & Apparel": 31.5, "Electronics": 27.8, "Home & Kitchen": 29.4,
    ...
    with bcol1:
        st.markdown(f"""<div class="panel"><div style="font-size:13px; color:{TEXT_SECONDARY};">{seller_name}</div>
            ...
            <div style="font-size:20px; font-weight:700; margin-top:8px;">{seller_abandon_rate:.1f}%</div></div>""", unsafe_allow_html=True)
    with bcol2:
        st.markdown(f"""<div class="panel"><div style="font-size:13px; color:{TEXT_SECONDARY};">Category average</div>
            ...
            <div style="font-size:20px; font-weight:700; margin-top:8px;">{benchmark_rate:.1f}%
                <span style="font-size:13px; color:{delta_color}; font-weight:700;"> ({'+' if delta > 0 else ''}{delta:.1f} pts)</span></div></div>""", unsafe_allow_html=True)
    ...
    with col_chart1:
        ...
        fig_feat = px.bar(feat_df, x='Impact Score', y='Factor', orientation='h', title='Leading drivers of abandonment',
    ...
    with col_chart2:
        ...
        fig_cart = px.bar(cart_grouped, x='Cart_Range', y='Abandonment (%)', title='Abandonment by cart value',
    ...
    st.download_button("Download session data (CSV)", data=csv_data, file_name=f"cartguard_{seller_name.replace(' ', '_')}.csv", mime="text/csv")

# ----------------- TAB 3: PRODUCT & TRENDS -----------------
with tab3:
    st.markdown(f'<p class="section-heading">Product & trends — {seller_name}</p>', unsafe_allow_html=True)

    trend_df = df_raw.groupby(df_raw['Date'].dt.date).agg(Sessions=('Abandoned', 'count'), Abandoned=('Abandoned', 'sum')).reset_index()
    trend_df['Abandonment Rate (%)'] = (trend_df['Abandoned'] / trend_df['Sessions'] * 100).round(1)
    trend_df = trend_df.sort_values('Date')

    fig_trend = go.Figure()
    fig_trend.add_trace(go.Scatter(x=trend_df['Date'], y=trend_df['Abandonment Rate (%)'], mode='lines',
                                    line=dict(color=ACCENT, width=2.5), fill='tozeroy', fillcolor='rgba(0,208,156,0.10)'))
    fig_trend.update_layout(title="Abandonment rate — 30 days", height=300, paper_bgcolor=SURFACE, plot_bgcolor=SURFACE,
    ...
    with col_funnel:
        ...
        funnel_stages = ["Viewed", "Cart", "Checkout", "Payment", "Order"]
        funnel_values = [int(total_sessions * 2.4), total_sessions,
        ...
        fig_funnel.update_layout(height=340, paper_bgcolor=SURFACE, plot_bgcolor=SURFACE, font_color=TEXT_PRIMARY, font_family=FONT, margin=dict(t=10, b=10))
...
with tab4:
    ...
    with col_s1:
        est_recovery_rate = st.slider("Recovery rate on at-risk carts (%)", min_value=5, max_value=50, value=20, step=5)
        est_avg_discount = st.slider("Average discount required (%)", min_value=0, max_value=25, value=10, step=5)
    with col_s2:
        net_margin_factor = 1 - (commission_rate / 100)
        ...
        st.info(f"**Projected outcome:**\n\n"
                f"- Carts recovered: **{saved_carts}**\n"
